...
import { RiBookOpenFill, RiMusic2Fill, RiCloseFill, RiCameraFill } from 'react-icons/ri'
  ...
  return (
    <React.Fragment>
      <div className={`font-poppins flex flex-col items-center w-full h-full overflow-x-hidden overflow-y-auto`}>
        <div
          className={`${!host || host.isLoggedIn === false ? 'max-w-5xl rounded-b-xl' : 'max-w-full'} relative flex w-full h-full max-h-[18rem] bg-center bg-cover bg-no-repeat bg-pantone-gray`}
        ...
        <div className={`${!host || host.isLoggedIn === false ? 'max-w-5xl' : 'max-w-full'} relative w-full`}>
          <div className="absolute -top-24 flex flex-col w-full px-3 lg:px-20 space-y-5">
            <div className="flex flex-col lg:flex-row items-center justify-center lg:justify-start w-full">
              <div className="relative flex w-full max-w-[11rem]">
                <img
                  ...
                  src={`${ get_profile.profile[0] ? `https://res.cloudinary.com/${process.env.CLOUD_NAME}/image/upload/v${get_profile.profile[0].version}/${get_profile.profile[0].publicId}.${get_profile.profile[0].format}` : `https://ui-avatars.com/api/?name=${get_profile.name}&background=1D1F21&color=FF3C3C` }`}
            ...
            <div className="relative flex flex-col lg:flex-row items-start w-full pt-0 lg:pt-3 pb-20 space-x-0 lg:space-x-2 space-y-2 lg:space-y-0">
              <div className={`${pathname === '/[username]/followers' || pathname === '/[username]/following' ? 'hidden lg:flex' : 'static lg:sticky flex'}  top-5 flex-col w-full max-w-full lg:max-w-xs h-full max-h-[30rem] p-5 space-y-5 rounded-xl overflow-hidden bg-pantone-darkblack`}>
